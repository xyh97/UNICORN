...
def load_video(video_path, n_frms=MAX_INT, height=-1, width=-1, sampling="uniform", s=0, e=-1, prob=1.0, pretrain=False, mask=False):
    ...
    if prob > 0.5:
        if not pretrain:
            ...
            start, end = int(s*fps), int(e*fps)
    elif prob == 0.5:
        start = max(0, int(s*fps)-5)
        end = min(int(e*fps)+5, vlen)
    else:
        if int(s*fps) > vlen:
            start, end = 0, vlen
        else:
            start = rnd.randint(0, int(s*fps))
            end = rnd.randint(min(int(e*fps), vlen), vlen)
            while start == end:
                start = rnd.randint(0, int(s*fps))
                end = rnd.randint(min(int(e*fps), vlen), vlen)
            crop = True
    if start == end:
        start = rnd.randint(0, start-1)
        end = rnd.randint(end, vlen-1)
        # start, end = 0, vlen
        # print(video_path, fps, vlen, start, end, s, e)
    if sampling == "uniform":
        # indices = np.arange(start, end, (end-start) / n_frms).astype(int)
        indices = np.linspace(start, end, n_frms, endpoint=False).astype(int)
        indices = np.clip(indices, 0, vlen-1)
        if len(indices) != n_frms:
            indices = indices[:n_frms]
            print(video_path, vlen, start, end)

...
class RunnerBase:
        ...
        if self._dataloaders is None:
            ...
            for split_name in self.datasets:
                if isinstance(self.datasets[split_name], tuple) or isinstance(self.datasets[split_name], list):
                    if len(self.datasets[split_name]) == 1:
                        self.datasets[split_name] = self.datasets[split_name][0]
    ...
    def train(self):
        ...
        latest_ckpt = os.path.join(self.output_dir, "checkpoint_latest.pth")
        if os.path.isfile(latest_ckpt):
            self._load_checkpoint(latest_ckpt)
            try:
                self.train_loader._epoch = self.start_epoch
                self.train_loader._dataloader.sampler.set_epoch(self.start_epoch)
                self.train_loader.iter_loader = iter(self.train_loader._dataloader)
            except:
                for i in range(len(self.train_loader.loaders)):
                    self.train_loader.loaders[i]._epoch = self.start_epoch
                    self.train_loader.loaders[i]._dataloader.sampler.set_epoch(self.start_epoch)
                    self.train_loader.loaders[i].iter_loader = iter(self.train_loader.loaders[i]._dataloader)
        for cur_epoch in range(self.start_epoch, self.max_epoch):
            ...
            if not self.evaluate_only:
                ...
                self._save_checkpoint(cur_epoch, is_latest=True)
                if (cur_epoch+1) % 4 == 0 and (cur_epoch+1) != self.max_epoch:
    ...
    @main_process
    def _save_checkpoint(self, cur_epoch, is_best=False, is_latest=False):
        """
    ...
    def log_stats(self, stats, split_name):
        if isinstance(stats, dict):
            ...
            # if split_name != "train":
            #     key = "best_epoch" if "best_epoch" in stats else "best_iters"
            #     with open(os.path.join(self.output_dir, "best.json"), "w") as f:
            #         json.dump({"agg_metrics": stats["agg_metrics"], "best": stats[key]}, f)
